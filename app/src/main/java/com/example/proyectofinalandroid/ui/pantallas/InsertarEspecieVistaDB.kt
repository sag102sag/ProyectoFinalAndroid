...
@Composable
fun AnyadirEspecieVista(
    especie: Especie,
    onAnyadirEspecie: (EspecieVistaDB) -> Unit
)
{
    ...
    var cantidadVista by remember { mutableStateOf(1) }
    ...
    Column(
        ...
        val especieNuevaDB = EspecieVistaDB(nombre = nombre, descripcion = descripcion, tipo = tipo, comentario = comentarios, fechaVisto = fechaFinal, cantidadVista = cantidadVista, favorito = favorito)

        Row {
            Button(onClick = { onAnyadirEspecie(especieNuevaDB) }) {
                Text(stringResource(R.string.a_adir_avistamiento))
            }
        }
    }
}
